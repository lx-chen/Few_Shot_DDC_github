...
data_path = join('/userhome/miniImageNet/miniImageNet')
savedir = './'
...
for dataset in dataset_list:
    with open(datasetmap[dataset] + ".csv", "r") as lines:
        # data_path_variable = os.path.join(data_path, datasetmap[dataset])
        data_path_variable = data_path + '/' + datasetmap[dataset]
        print("data_path_variable",data_path_variable)        
        for i, line in enumerate(lines):
            if i == 0:
                continue
            fid, _ , label = re.split(',|\.', line)   # 它使用逗号 , 或者点号 . 作为分隔符，将 line 拆分成多个部分
            # print(fid)
            # print(label)
            label = label.replace('\n','')
            if not label in filelists[dataset]:
                ...
                # fnames = listdir(join(data_path_variable, label))
                fnames = listdir(data_path_variable + '/' + label)
                # print("fnames",fnames)
            ...
            for i in sorted_fnames:
                # fname = join( data_path_variable,label, i)
                fname = data_path_variable + '/' + label + '/' + i
                # print(fname)
