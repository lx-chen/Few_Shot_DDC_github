...
class FSLTaskMaker:
    ...
    def loadDataSet(self, dsname, features_dir):
        ...
        self._randStates = None
        self._rsCfg = None
        assert os.path.exists(features_dir), f'{features_dir} does not exist'  # 检查特征文件夹是否存在
        ...
        # Computing the number of items per class in the dataset 确定数据集中每个类别的最小样本数。这样做的目的是保证每个类别都有相同数量的样本，以便后续操作。
        ...
        # Generating data tensors  根据计算得到的每个类别的最小样本数，从加载的数据集中选择相应数量的样本，组成新的数据张量。
