...
def Distribution_fitting_with_DDWM2(query, base_means, base_means_matrix, base_cov, k, alpha, gamma, a, b, c, T): # base_means.shape=(64,640)
    assert torch.is_tensor(query)
    assert torch.is_tensor(base_means)
    assert torch.is_tensor(base_cov)
    # print("query------------------------",query)
    ...
    for setting in itertools.product(firth_coeff_list, n_ways_list, n_shots_list, n_aug_list, split_name_list):
        ...
        for ii, run_idxs in enumerate(all_run_idxs):  # 用于执行元学习中的任务
            ...
            with torch.no_grad():
                ...
                mean_tch, cov_tch = Distribution_fitting_with_DDWM2(support_data, base_means, base_means_matrix, base_cov,
